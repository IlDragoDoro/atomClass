...
<html lang="en" dir="ltr">
  <head>
    ...
    <title>Libros de actualidad</title>
  </head>
  <?php
  ...
  <body>
    ...
    <form class="form_Search" action="buscar.php" method="get">
      <input  class="search_bar" type="text" name="busqueda" id="busqueda" placeholder="<?php echo $busqueda ?>">
...
<?php foreach($conn->query ("SELECT * FROM libro WHERE titulo LIKE '%$busqueda%' OR autor LIKE '%$busqueda%' OR editorial LIKE '%$busqueda%' or genero LIKE '%$busqueda%' ") as $row){?>
  <tr>
    <td><?php $genero=$row['genero']; $img=$row['img']; echo "<img src='img/libros/$genero/$img' width='50px'>" ?></td>
